    ...
    for (map<uint256, CWalletTx>::const_iterator it = mapWallet.begin(); it != mapWallet.end(); ++it) {
        ...
        for (unsigned int i = 0; i < wtx.vout.size(); i++) {
            ...
            if(fAnonymizable) {
                ...
                if(fMasterNode && wtx.vout[i].nValue == Params(CBaseChainParams::MAIN).GetConsensus().masternodeCount) continue;
...
bool CWallet::SelectCoinsDark(CAmount nValueMin, CAmount nValueMax, std::vector<CTxIn>& vecTxInRet, CAmount& nValueRet, int nPrivatePayRoundsMin, int nPrivatePayRoundsMax) const
{
    ...
    BOOST_FOREACH(const COutput& out, vCoins)
    {
        ...
        if(fMasterNode && out.tx->vout[out.i].nValue == Params(CBaseChainParams::MAIN).GetConsensus().masternodeCount) continue; //masternode input
            ...
            while (true)
            {
                ...
                if (!SelectCoins(nValueToSelect, setCoins, nValueIn, coinControl, nCoinType, fUseInstaPay))
                {
                    if (nCoinType == ONLY_NOT100000IFMN) {
                        strFailReason = _("Unable to locate enough funds for this transaction that are not equal 1000 OIOC.");
                    } else if (nCoinType == ONLY_NONDENOMINATED_NOT100000IFMN) {
                        strFailReason = _("Unable to locate enough PrivatePay non-denominated funds for this transaction that are not equal 1000 OIOC.");
                    ...
                    if (fUseInstaPay) {
                        if (nValueIn > sporkManager.GetSporkValue(SPORK_5_INSTAPAY_MAX_VALUE)*COIN) {
                            strFailReason += " " + strprintf(_("InstaPay doesn't support sending values that high yet. Transactions are currently limited to %1 OIOC."), sporkManager.GetSporkValue(SPORK_5_INSTAPAY_MAX_VALUE));
                        } else {
                ...
                if (nChange > 0)
                {
                    ...
                    if (nCoinType == ONLY_DENOMINATED) {
                        ...
                        // change transaction isn't always pay-to-OIOCoin-address
                        CScript scriptChange;
