...
#if defined(NDEBUG)
# error "OIOCoin Core cannot be compiled without assertions."
#endif
...
const string strMessageMagic = "OIOCoin Signed Message:\n";
...
CAmount GetBlockSubsidy(int nPrevBits, int nPrevHeight, const Consensus::Params& consensusParams, bool fSuperblockPartOnly)
{
    ...
    }else if(nPrevHeight <= 525600){
        nSubsidy = 5707 *COIN;  //30 0000 0000
    }else if(nPrevHeight <= 525600 * 2){
        nSubsidy = 3805 *COIN; //20 0000 0000
    }else if(nPrevHeight <= 525600 * 3){
        nSubsidy = 1902 *COIN; //10 0000 0000
    }else{
        nSubsidy = 190 *COIN;
    }
...
{
    CAmount ret = blockValue*15/100; // start at 15%
    if(nHeight > Params(CBaseChainParams::MAIN).GetConsensus().nForkHeight){
...
bool ConnectBlock(const CBlock& block, CValidationState& state, CBlockIndex* pindex, CCoinsViewCache& view, bool fJustCheck)
{
    ...
    // OIOC : MODIFIED TO CHECK MASTERNODE PAYMENTS AND SUPERBLOCKS
    ...
    if(pindex->pprev->nHeight == chainparams.GetConsensus().nForkHeight){
        blockReward+=100000000*COIN;
    }
    std::string strError = "";
    if (!IsBlockValueValid(block, pindex->nHeight, blockReward, strError)) {
        return state.DoS(0, error("ConnectBlock(OIOC): %s", strError), REJECT_INVALID, "bad-cb-amount");
    }

    if (!IsBlockPayeeValid(block.vtx[0], pindex->nHeight, blockReward)) {
        mapRejectedBlocks.insert(make_pair(block.GetHash(), GetTime()));
        return state.DoS(0, error("ConnectBlock(OIOC): couldn't find masternode or superblock payments"),
...
bool CheckBlock(const CBlock& block, CValidationState& state, bool fCheckPOW, bool fCheckMerkleRoot)
{
    ...
    // OIOC : CHECK TRANSACTIONS FOR INSTAPAY
    ...
    if(sporkManager.IsSporkActive(SPORK_3_INSTAPAY_BLOCK_FILTERING)) {
        ...
        BOOST_FOREACH(const CTransaction& tx, block.vtx) {
            ...
            BOOST_FOREACH(const CTxIn& txin, tx.vin) {
                ...
                if(instapay.GetLockedOutPointTxHash(txin.prevout, hashLocked) && hashLocked != tx.GetHash()) {
                    ...
                    LOCK(cs_main);
                    mapRejectedBlocks.insert(make_pair(block.GetHash(), GetTime()));
                    return state.DoS(0, error("CheckBlock(OIOC): transaction %s conflicts with transaction lock %s",
        ...
        LogPrintf("CheckBlock(OIOC): spork is off, skipping transaction locking checks\n");
    }
...
bool ContextualCheckBlockHeader(const CBlockHeader& block, CValidationState& state, CBlockIndex * const pindexPrev)
{
    ...
    if(Params().NetworkIDString() == CBaseChainParams::MAIN && nHeight <= 68589){
        ...
        if (block.nBits != GetNextWorkRequired(pindexPrev, &block, consensusParams) && pindexPrev->nHeight >= consensusParams.nForkHeight)
...
static bool AcceptBlockHeader(const CBlockHeader& block, CValidationState& state, const CChainParams& chainparams, CBlockIndex** ppindex)
{
    ...
    if (hash != chainparams.GetConsensus().hashGenesisBlock) {
        ...
        if (miSelf != mapBlockIndex.end()) {
            ...
            if (pindex->nHeight < chainparams.GetConsensus().nForkHeight){
                return true;
            }
        ...
        if (pindexPrev->nStatus & BLOCK_FAILED_MASK && pindex->nHeight > chainparams.GetConsensus().nForkHeight)
