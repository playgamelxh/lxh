...
const char * const BITCOIN_CONF_FILENAME = "OIOCoin.conf";
const char * const BITCOIN_PID_FILENAME = "OIOCoind.pid";
...
bool LogAcceptCategory(const char* category)
{
    if (category != NULL)
    {
        ...
        if (ptrCategory.get() == NULL)
        {
            ...
            // "OIOCoin" is a composite category enabling all OIOCoin-related debug output
            if(ptrCategory->count(string("OIOCoin"))) {
...
boost::filesystem::path GetDefaultDataDir()
{
    ...
    // Windows < Vista: C:\Documents and Settings\Username\Application Data\OIOCoin
    // Windows >= Vista: C:\Users\Username\AppData\Roaming\OIOCoin
    // Mac: ~/Library/Application Support/OIOCoin
    // Unix: ~/.OIOCoin
#ifdef WIN32
    // Windows
    return GetSpecialFolderPath(CSIDL_APPDATA) / "OIOCoin";
#else
    fs::path pathRet;
...
#ifdef MAC_OSX
    // Mac
    return pathRet / "Library/Application Support/OIOCoin";
#else
    // Unix
    return pathRet / ".OIOCoin";
#endif
#endif
}
                    ...
                    map<string, vector<string> >& mapMultiSettingsRet)
{
    ...
    if (!streamConfig.good()){
        // Create empty OIOCoin.conf if it does not excist
    ...
    {
        // Don't overwrite existing settings so command line settings override OIOCoin.conf
