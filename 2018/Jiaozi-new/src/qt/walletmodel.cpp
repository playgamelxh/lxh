...
WalletModel::SendCoinsReturn WalletModel::prepareTransaction(WalletModelTransaction &transaction, const CCoinControl *coinControl)
{
    ...
    Q_FOREACH(const SendCoinsRecipient &rcp, recipients)
    {
        ...
        {   // User-entered OIOCoin address / amount:
...

    {
        ...
        if(recipients[0].fUseInstaPay && total > sporkManager.GetSporkValue(SPORK_5_INSTAPAY_MAX_VALUE)*COIN){
            Q_EMIT message(tr("Send Coins"), tr("InstaPay doesn't support sending values that high yet. Transactions are currently limited to %1 OIOC.").arg(sporkManager.GetSporkValue(SPORK_5_INSTAPAY_MAX_VALUE)),
...
WalletModel::SendCoinsReturn WalletModel::sendCoins(WalletModelTransaction &transaction)
{
...

    {
        ...
        Q_FOREACH(const SendCoinsRecipient &rcp, recipients)
        {
            ...
            else if (!rcp.message.isEmpty()) // Message from normal OIOCoin:URI (OIOCoin:XyZ...?message=example)
            {
